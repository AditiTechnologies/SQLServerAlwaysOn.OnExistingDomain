...
function Set-TargetResource
{
    ...
    $primarySqlNodeName = $PrimaryDatacenterSQLNodeNamePrefix + '1'
    $disasterRecoverySqlNodeName = $env:COMPUTERNAME
    $agExist = Check-SQLHAGroup -InstanceName $primarySqlNodeName -Name $Name -sa $sa -saPassword $saPassword    
...
"@
        for($index = 1; $index -le $NumberOfPrimaryDatacenterSQLNodes; $index++)
        {
            ...
            $queryAdd = @"
                        $queryAdd
                        '$PrimaryDatacenterSQLNodeNamePrefix$index' WITH
                        (
                            ENDPOINT_URL = 'TCP://${PrimaryDatacenterSQLNodeNamePrefix}${index}.${domain}:${EndpointPort}',
...
"@
        ...
        $queryAdd = @"
                        $queryAdd
                        '$disasterRecoverySqlNodeName' WITH
                        (
                            ENDPOINT_URL = 'TCP://${disasterRecoverySqlNodeName}.${domain}:${EndpointPort}',
                            FAILOVER_MODE = $ManualFailoverMode,
                            AVAILABILITY_MODE = $AsynchronousCommitAvailabilityMode
                        )
"@
        $query = $query.Trim() + $queryAdd.Trim()
...
function Test-TargetResource
{
    ...
    $primarySqlNodeName = $PrimaryDatacenterSQLNodeNamePrefix + '1'
    $bFound = Check-SQLHAGroup -InstanceName $primarySqlNodeName -Name $Name -sa $sa -saPassword $saPassword
    if ($bFound)
    {
